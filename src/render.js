const render = (game) =>{
    /*if(getNumberPerSecond(game).toString().split(".").length != 0){
    ...
    stats2DOM.innerHTML = `Essence: ${numberformat.format(game.essence,{sigfigs:3})} | Essence cost: ${numberformat.format(getEssenceCost(game))} <strong>BN</strong> | Auto Numberers: ${game.auto_numbers} | Auto Numberer Price: ${getAutoNumberCost(game)} <strong>E</strong> | Number Per Second: ${getNumberPerSecond(game).toFixed(1)}`;
